...
export interface PoliceTokenAnswer {
  ...
  turnNumber: number;
  /**
   * Обстановка в момент вопроса — всё видно на доске, но потом меняется.
   * По ней ответ можно сверить с каждым мотивом: «мог убить» исключает мотивы,
   * которые эту жертву запрещают. У старых сохранений поля нет.
   */
  context?: {
    districtX: number;
    districtY: number;
    isScared: boolean;
    carAt: { x: number; y: number } | null;
    /** Живых в районе жителя, вместе с ним */
    population: number;
    /** Сколько убийств уже было */
    victimsCount: number;
  };
}
...
export interface Victim {
  ...
  wasScared: boolean;
  /**
   * Где стояла машина детектива в ночь убийства и сколько живых было в районе
   * жертвы (вместе с ней). Оба факта детектив видел на доске, но после убийства
   * машина уезжает, а жители расселяются — без записи их не восстановить.
   * Нужны для мотивов «Вигилант» и «Киллер». У старых сохранений полей нет.
   */
  carAt?: { x: number; y: number } | null;
  populationAtKill?: number;
}
